...
@router.get("/gas")
async def gas():
    """Return the latest natural gas price"""
    latest_data.main()
    with engine.connect() as conn:
        row = conn.execute(
            text('SELECT * FROM gas ORDER BY "Date" DESC LIMIT 1;')
        ).fetchone()
    return {
        "symbol": "NG",
        "date": row[0].date(),
...
async def gas_history():
    ...
    latest_data.main()
    df = pd.read_sql('SELECT "Date", "Close" FROM gas ORDER BY "Date"', engine)
    df["Date"] = pd.to_datetime(df["Date"])

    plt.figure(figsize=(20, 8))
    plt.plot(df["Date"], df["Close"], color="blue")
    plt.title("5 Year Gas Price in USD/MMBtu")
    plt.xlabel("Year")
    plt.ylabel("Price")
    plt.savefig("gas_history.png", bbox_inches="tight")

    image_path = Path("gas_history.png")
    return FileResponse(image_path)


@router.get("/gas/analytics")
async def gas_analytics():
